...
enum LocationAvailability: Sendable {
  ...
  /**
   ...
   A stream that failed outright reports this too, for the same reason: there is no position and
   nothing the pilot can do to produce one.
   */
...
extension LocationStreamer {
  /**
   Whether asking Core Location again could change the answer.

   True of a refusal the pilot may since have lifted in Settings, and of a stream that failed and
   left nothing running. Drives the retry that runs when the app returns to the foreground.
   */
  var needsFreshStream: Bool {
    availability?.deniesAuthorization == true || error != nil
  }
}
...
final class CoreLocationStreamer: LocationStreamer {
  ...
  func locationUpdates() -> AsyncStream<CLLocation> {
    AsyncStream { continuation in
      let id = UUID()
      // Registered before the stream is returned, and so before anything can iterate it. A
      // subscriber whose task is already cancelled terminates the stream on its first `next()`,
      // and a handler assigned after termination is never called — which would count a listener
      // that nothing can ever release, and leave the GPS running with no one listening.
  ...
  private func beginUpdates() {
    ...
    updateTask = Task {
      ...
      do {
        ...
        self.error = error
        // Nothing restarts a stream that threw, so the tab needs a state it can draw. Left as it
        // was, a failure on the first attempt would hold the spinner for the life of the process.
